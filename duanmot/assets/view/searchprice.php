...
<script>
  ...
  if ($result->rowCount() > 0) {
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      echo '<a href="product_detail.php?id=' . $row['id'] . '"><img class="product-img" src="../img/' . $row['img'] . '" alt="Product Image"></a>';
...
<div class="product">
 ...
 if (isset($_GET['filterPrice'])) {
    ...
    // Sản phẩm chính 
    $query = "SELECT id, name, price, img FROM product WHERE price <= :filterPrice";
    $stmt = $conn->prepare($query);
    $stmt->bindValue(':filterPrice', $filterPrice, PDO::PARAM_INT);
    $stmt->execute();
              echo '<h2>Kết quả tìm kiếm cho: ' .  $filterPrice. ' đ</h2>';
